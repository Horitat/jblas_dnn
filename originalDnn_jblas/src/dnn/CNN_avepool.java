...
public class CNN_avepool {
	...
	public CNN_avepool(int[] imageSize, int channel, int[] nKernels, int minibatch,
		...
		pool = new AvePoolinglayer[conv.length];
...
//			System.out.println("poolout:"+ convoutsize[i][0] / poolSizes[i][0] +","+ convoutsize[i][1] / poolSizes[i][1]);
			...
			pool[i] = new AvePoolinglayer(poolkernelsize[i],pooloutsize[i], nKernels[i], 1, minibatch,mt, "AVE", "");
		}
	...
	public static void main(String[] args) {
		...
		CNN_avepool classifier = new CNN_avepool(imageSize, channel, nKernels,minibatchSize, kernelSizes, poolSizes, nHidden, nOut, mt, "ReLU");
	...
	public void train(FloatMatrix[][] x, FloatMatrix label, int minibatchsize, float l_rate){
		...
		for(int i= kernelnum.length-1; i>-1; i--){
			FloatMatrix[][] poolback = pool[i].backavepooing(dc, convoutsize[i], minibatchsize);
