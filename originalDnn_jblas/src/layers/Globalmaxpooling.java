...
public class Globalmaxpooling {
	...
	public Globalmaxpooling(int channel,int minibatch){
	...
	public FloatMatrix[][] backward(FloatMatrix dy){
		...
		for(int m=0; m< back.length; m++){
			for(int c=0; c<back[m].length; c++){
				back[m][c] = input_data[m][c].eq(output_data.get(m,c)).mul(dy.get(m,c));
			}
		}
		return back;
	}
